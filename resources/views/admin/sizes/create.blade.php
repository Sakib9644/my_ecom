...
<div class="max-w-xl bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
    <form action="{{ route('admin.sizes.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white" placeholder="e.g. XL, US 10, 42mm">
            @error('name')
        ...
        <div>
            ...
            <select name="category" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white">
                ...
                <option value="shoe" {{ old('category') == 'shoe' ? 'selected' : '' }}>Shoe</option>
