    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" id="product-form">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" name="name" value="{{ old('name') }}" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white" placeholder="e.g. Wireless Noise Canceling Headphones">
            ...
            <div>
                ...
                <input type="number" name="price" step="0.01" value="{{ old('price') }}" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white" placeholder="0.00">
            ...
            <div id="default-stock-field">
                ...
                <input type="number" name="stock" value="{{ old('stock', 0) }}" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white" placeholder="0">
                        ...
                        @foreach($sizes as $size)
                            <label class="flex items-center gap-2 p-2.5 rounded-xl border-2 border-slate-200 cursor-pointer hover:border-primary-400 hover:bg-primary-50/30 transition-all has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50">
                                <input type="checkbox" name="sizes[]" value="{{ $size->id }}"
                                    {{ old('sizes') && in_array($size->id, old('sizes')) ? 'checked' : '' }}
        ...
        <div class="border border-slate-200 rounded-2xl overflow-hidden">
            ...
            <div class="p-4 space-y-4">
                ...
                @else
                    ...
                    <div id="color-images-section" class="border-t border-slate-100 pt-4 hidden">
        ...
        <div>
            ...
            <textarea name="description" rows="4" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white" placeholder="Product features, sizing guide, care instructions…">{{ old('description') }}</textarea>
            @error('description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <!-- Featured Checkbox -->
        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="rounded border-slate-300 text-primary-600 focus:ring-primary-500 h-4 w-4">
        ...
        <div class="pt-4 border-t border-slate-100 flex gap-4 justify-end">
            ...
            <button type="submit" class="px-5 py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl text-xs transition-colors shadow-md shadow-primary-500/10">Save Product</button>
        </div>
    </form>
</div>
@endsection
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function buildVariantStock() {
            ...
            if (hasColors && hasSizes) {
                // Grid: each color with each size
                ...
                colors.forEach(color => {
                    ...
                    let sizeInputs = sizes.map(size => `
                        <div class="flex items-center gap-2">
                            ...
                            <input type="number" name="variant_stock[${color.name}|${size}]" min="0" value="0" class="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-primary-500 bg-white" placeholder="0">
                        </div>
                    `).join('');
                    card.innerHTML = `
                ...
                colors.forEach(color => {
                    ...
                    row.innerHTML = `
                        ...
                        <input type="number" name="variant_stock[${color.name}]" min="0" value="0" class="flex-1 rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-primary-500 bg-white" placeholder="Stock quantity">
                    `;
                ...
                sizes.forEach(size => {
                    ...
                    row.innerHTML = `
                        ...
                        <input type="number" name="variant_stock[${size}]" min="0" value="0" class="flex-1 rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-primary-500 bg-white" placeholder="Stock quantity">
                    `;
                    variantGrid.appendChild(row);
                });
            }
