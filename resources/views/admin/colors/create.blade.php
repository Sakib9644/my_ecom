...
@section('page_title', 'Add New Color')
...
<div class="max-w-xl bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
    <form action="{{ route('admin.colors.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white" placeholder="e.g. Matte Black, Rose Gold, Midnight Blue">
            @error('name')
        ...
        <div>
            ...
            <div class="flex items-center gap-3">
                <input type="color" id="color-picker" value="{{ old('hex_code', '#0ea5e9') }}" class="h-10 w-16 rounded-xl border border-slate-300 cursor-pointer bg-white p-1">
                <input type="text" name="hex_code" id="hex-code-input" value="{{ old('hex_code', '') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white" placeholder="#1e293b">
            </div>
        ...
        <div>
            ...
            <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white" placeholder="0">
            @error('sort_order')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
...
<script>
    ...
    // Sync text input → picker (only if valid hex)
